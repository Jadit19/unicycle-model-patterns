...
void odometryCallback(const nav_msgs::OdometryConstPtr& msg){
    ...
    if (state == FSM::MOVE){
        ...
        if (abs(msg->pose.pose.position.x - R_MIN) < TOLERANCE){
            std::cout << std::endl;
            startingpositionx = msg->pose.pose.position.x;
            startingpositiony = msg->pose.pose.position.y;
        ...
        double alphaC = atan((msg->pose.pose.position.y - startingpositiony)/(msg->pose.pose.position.x - startingpositionx));
        double errorAlpha = alphaC - alphaR;
        V = VELOCITY_GAIN*cos(errorAlpha)*r;
        if (state == FSM::TRAJECTORY_1){
            cmd_vel.angular.z = f1(r);
            cmd_vel.angular.z = ALPHA_GAIN*errorAlpha + cmd_vel.angular.z;
        }
        else if (state == FSM::TRAJECTORY_2){
...
int main(int argc, char** argv){
    ...
    || !nh.getParam("velocity_gain", VELOCITY_GAIN) || !nh.getParam("alpha_gain", ALPHA_GAIN)){
        ROS_ERROR("\nCan't load params..");
